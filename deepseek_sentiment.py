...
def setup_openai_client(api_key):
    ...
    return OpenAI(
        api_key=api_key,
        base_url="http://44.197.202.56:8000/v1",  # DeepSeek vLLM API server
    )
...
def upload_to_s3(df, s3_client, bucket_name, file_key):
    ...
    output_key = f"{folder_path}/stock_news_sentiments.csv" if folder_path else "stock_news_sentiments.csv"
